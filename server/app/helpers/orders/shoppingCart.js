	...
	async addItemToCart(itemId, userId) {
		let result = await connectionPool.executePreparedSqlInstruction(
			[
				...
				"VALUES ((SELECT ID FROM PEDIDO WHERE PEDIDO.USUARIO_ID = ? AND PEDIDO.STATUS_ID = (SELECT ID FROM PEDIDO_STATUS WHERE PEDIDO_STATUS.NOME_EXIBICAO = 'CONCLUIDO') FETCH FIRST 1 ROW ONLY), ?, 0)",
				"));"
			].join(" "),
