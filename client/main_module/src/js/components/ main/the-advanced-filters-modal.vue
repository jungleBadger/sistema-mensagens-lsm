		...
		<lsm-modal
			...
			<template v-slot:modal-content>
				...
				<div
					...
					<div class="flex flex-col gap-1" v-if="isLoggedIn">
						<div class="w-full md:w-80 flex flex-col gap-1">
							<label class="text-gray-700 ">Mensagens adquiridas</label>
							<lsm-checkbox
								v-model="localOwnedOnly"
								:label="localOwnedOnly ? 'Apenas mensagens que comprei' : 'Todas mensagens'"
							></lsm-checkbox>
						</div>
					</div>
...
export default defineComponent({
	...
	"computed": {
		...
		"ownedOnly": {
			get () {
				return this.$store.getters["advancedFilters/ownedOnly"];
			},
			set (val) {
				return this.$store.commit("advancedFilters/ownedOnly", val);
			}
		},
	...
	"methods": {
		...
		applyFilters () {
			...
			this.ownedOnly = this.localOwnedOnly;
	...
	mounted () {
		...
		this.localOwnedOnly = this.ownedOnly
